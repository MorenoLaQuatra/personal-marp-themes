...
Good for citations or memorable lines (not really used unless necessary)
...
Tables are left-aligned by default, sized to their content, not centered. This is deliberate: a full-width or nearly-full-width table often needs to sit flush left, so the theme does not force centering on every table.
To center a narrower table on the slide, wrap it in native `<center>...</center>` tags:
...
This is opt-in per table, not a global default.
...
- [ ] HTML is limited to single wrapper `<div>`s, `<figure>`/`<figcaption>`, and `<span>` for inline color/value emphasis
- [ ] Box classes used match their semantic meaning (definition, example, important, warning, note)
